...
import 'package:masterpie/feature/foods/presentation/screen/ui_helper/meal_plan_ready_popup.dart';
...
class _MainScreenState extends State<MainScreen> with SingleTickerProviderStateMixin{
  ...
  bool _addMainDishClicked= false;
  bool _addSideDishClicked= false;

  String _selectedDishType= '';

  String _diet= CLASSIC_LABEL;

  bool _isSimplePlanSelected= true;
  ...
  void setDiet(String diet){
    setState(() {
      ...
      if(diet == KETO_LABEL){
        _proteinPercentageController= TextEditingController(text: '25');
        _carbPercentageController= TextEditingController(text: '5');
        _fatPercentageController= TextEditingController(text: '70');
      }else{
        _proteinPercentageController= TextEditingController(text: '30');
        _carbPercentageController= TextEditingController(text: '40');
        _fatPercentageController= TextEditingController(text: '30');
  ...
  void _setAllAutoGeneratedMealsInWizard(List<Food> foods){
    setState(() {
      ...
      for(int i = _mainDishTypes.length; i < foods.length; i++){
        if(i < foods.length){
    ...
    if(isFromOnboard == true){
      ...
      showDialog(
        ...
        barrierDismissible: false,
        builder: (BuildContext context) {
          return MealPlanReadyPopup(readyForTutorial: ready,);
        },
      );
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              FoodsMacroListUi(mainDishesFoods: _mainDishFoods, sideDishesFoods: _sideDishFoods, mainDishesTypes: _mainDishTypes, sideDishesTypes: _sideDishTypes,
                onRemoveDishClicked: updateUiAfterDishRemoved, onRemoveFoodClicked: updateUiAfterFoodRemoved, onMainDishClicked: _onMinDishClicked, onSideDishClicked: _onSideDishClicked,
              )
            ],
          ),
        )
  ...
  void setMacroGoals(List<String> dailyMacroGoal){
    setState(() {
      ...
      _diet= dailyMacroGoal[4].isEmpty ? CLASSIC_LABEL : dailyMacroGoal[4];
      setDiet(_diet);
    });
  }
