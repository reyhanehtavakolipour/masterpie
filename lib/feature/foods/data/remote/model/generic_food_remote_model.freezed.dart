// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'generic_food_remote_model.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
    'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#custom-getters-and-methods');

/// @nodoc
mixin _$GenericFoodRemote {
  String get id => throw _privateConstructorUsedError;
  FoodTypeRemote get foodType => throw _privateConstructorUsedError;
  String get name => throw _privateConstructorUsedError;
  String get barcode => throw _privateConstructorUsedError;
  String get brandName => throw _privateConstructorUsedError;
  String get description => throw _privateConstructorUsedError;
  String get nationality => throw _privateConstructorUsedError;
  String get image => throw _privateConstructorUsedError;
  List<double> get servingAmount => throw _privateConstructorUsedError;
  List<String> get unit => throw _privateConstructorUsedError;
  List<String> get ingredients => throw _privateConstructorUsedError;
  List<List<String>> get servingIngredientsCount =>
      throw _privateConstructorUsedError;
  String get recipe => throw _privateConstructorUsedError;
  List<List<String>> get units => throw _privateConstructorUsedError;
  List<List<String>> get calorie => throw _privateConstructorUsedError;
  List<List<String>> get protein => throw _privateConstructorUsedError;
  List<List<String>> get carb => throw _privateConstructorUsedError;
  List<List<String>> get fat => throw _privateConstructorUsedError;
  List<String> get diets => throw _privateConstructorUsedError;
  List<String> get allergies => throw _privateConstructorUsedError;
  double get count => throw _privateConstructorUsedError;
  String get prepTime => throw _privateConstructorUsedError;
  int get createdFromFatSecretRecipes => throw _privateConstructorUsedError;

  @JsonKey(ignore: true)
  $GenericFoodRemoteCopyWith<GenericFoodRemote> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $GenericFoodRemoteCopyWith<$Res> {
  factory $GenericFoodRemoteCopyWith(
          GenericFoodRemote value, $Res Function(GenericFoodRemote) then) =
      _$GenericFoodRemoteCopyWithImpl<$Res, GenericFoodRemote>;
  @useResult
  $Res call(
      {String id,
      FoodTypeRemote foodType,
      String name,
      String barcode,
      String brandName,
      String description,
      String nationality,
      String image,
      List<double> servingAmount,
      List<String> unit,
      List<String> ingredients,
      List<List<String>> servingIngredientsCount,
      String recipe,
      List<List<String>> units,
      List<List<String>> calorie,
      List<List<String>> protein,
      List<List<String>> carb,
      List<List<String>> fat,
      List<String> diets,
      List<String> allergies,
      double count,
      String prepTime,
      int createdFromFatSecretRecipes});
}

/// @nodoc
class _$GenericFoodRemoteCopyWithImpl<$Res, $Val extends GenericFoodRemote>
    implements $GenericFoodRemoteCopyWith<$Res> {
  _$GenericFoodRemoteCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? id = null,
    Object? foodType = null,
    Object? name = null,
    Object? barcode = null,
    Object? brandName = null,
    Object? description = null,
    Object? nationality = null,
    Object? image = null,
    Object? servingAmount = null,
    Object? unit = null,
    Object? ingredients = null,
    Object? servingIngredientsCount = null,
    Object? recipe = null,
    Object? units = null,
    Object? calorie = null,
    Object? protein = null,
    Object? carb = null,
    Object? fat = null,
    Object? diets = null,
    Object? allergies = null,
    Object? count = null,
    Object? prepTime = null,
    Object? createdFromFatSecretRecipes = null,
  }) {
    return _then(_value.copyWith(
      id: null == id
          ? _value.id
          : id // ignore: cast_nullable_to_non_nullable
              as String,
      foodType: null == foodType
          ? _value.foodType
          : foodType // ignore: cast_nullable_to_non_nullable
              as FoodTypeRemote,
      name: null == name
          ? _value.name
          : name // ignore: cast_nullable_to_non_nullable
              as String,
      barcode: null == barcode
          ? _value.barcode
          : barcode // ignore: cast_nullable_to_non_nullable
              as String,
      brandName: null == brandName
          ? _value.brandName
          : brandName // ignore: cast_nullable_to_non_nullable
              as String,
      description: null == description
          ? _value.description
          : description // ignore: cast_nullable_to_non_nullable
              as String,
      nationality: null == nationality
          ? _value.nationality
          : nationality // ignore: cast_nullable_to_non_nullable
              as String,
      image: null == image
          ? _value.image
          : image // ignore: cast_nullable_to_non_nullable
              as String,
      servingAmount: null == servingAmount
          ? _value.servingAmount
          : servingAmount // ignore: cast_nullable_to_non_nullable
              as List<double>,
      unit: null == unit
          ? _value.unit
          : unit // ignore: cast_nullable_to_non_nullable
              as List<String>,
      ingredients: null == ingredients
          ? _value.ingredients
          : ingredients // ignore: cast_nullable_to_non_nullable
              as List<String>,
      servingIngredientsCount: null == servingIngredientsCount
          ? _value.servingIngredientsCount
          : servingIngredientsCount // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      recipe: null == recipe
          ? _value.recipe
          : recipe // ignore: cast_nullable_to_non_nullable
              as String,
      units: null == units
          ? _value.units
          : units // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      calorie: null == calorie
          ? _value.calorie
          : calorie // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      protein: null == protein
          ? _value.protein
          : protein // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      carb: null == carb
          ? _value.carb
          : carb // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      fat: null == fat
          ? _value.fat
          : fat // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      diets: null == diets
          ? _value.diets
          : diets // ignore: cast_nullable_to_non_nullable
              as List<String>,
      allergies: null == allergies
          ? _value.allergies
          : allergies // ignore: cast_nullable_to_non_nullable
              as List<String>,
      count: null == count
          ? _value.count
          : count // ignore: cast_nullable_to_non_nullable
              as double,
      prepTime: null == prepTime
          ? _value.prepTime
          : prepTime // ignore: cast_nullable_to_non_nullable
              as String,
      createdFromFatSecretRecipes: null == createdFromFatSecretRecipes
          ? _value.createdFromFatSecretRecipes
          : createdFromFatSecretRecipes // ignore: cast_nullable_to_non_nullable
              as int,
    ) as $Val);
  }
}

/// @nodoc
abstract class _$$GenericFoodRemoteImplCopyWith<$Res>
    implements $GenericFoodRemoteCopyWith<$Res> {
  factory _$$GenericFoodRemoteImplCopyWith(_$GenericFoodRemoteImpl value,
          $Res Function(_$GenericFoodRemoteImpl) then) =
      __$$GenericFoodRemoteImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call(
      {String id,
      FoodTypeRemote foodType,
      String name,
      String barcode,
      String brandName,
      String description,
      String nationality,
      String image,
      List<double> servingAmount,
      List<String> unit,
      List<String> ingredients,
      List<List<String>> servingIngredientsCount,
      String recipe,
      List<List<String>> units,
      List<List<String>> calorie,
      List<List<String>> protein,
      List<List<String>> carb,
      List<List<String>> fat,
      List<String> diets,
      List<String> allergies,
      double count,
      String prepTime,
      int createdFromFatSecretRecipes});
}

/// @nodoc
class __$$GenericFoodRemoteImplCopyWithImpl<$Res>
    extends _$GenericFoodRemoteCopyWithImpl<$Res, _$GenericFoodRemoteImpl>
    implements _$$GenericFoodRemoteImplCopyWith<$Res> {
  __$$GenericFoodRemoteImplCopyWithImpl(_$GenericFoodRemoteImpl _value,
      $Res Function(_$GenericFoodRemoteImpl) _then)
      : super(_value, _then);

  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? id = null,
    Object? foodType = null,
    Object? name = null,
    Object? barcode = null,
    Object? brandName = null,
    Object? description = null,
    Object? nationality = null,
    Object? image = null,
    Object? servingAmount = null,
    Object? unit = null,
    Object? ingredients = null,
    Object? servingIngredientsCount = null,
    Object? recipe = null,
    Object? units = null,
    Object? calorie = null,
    Object? protein = null,
    Object? carb = null,
    Object? fat = null,
    Object? diets = null,
    Object? allergies = null,
    Object? count = null,
    Object? prepTime = null,
    Object? createdFromFatSecretRecipes = null,
  }) {
    return _then(_$GenericFoodRemoteImpl(
      id: null == id
          ? _value.id
          : id // ignore: cast_nullable_to_non_nullable
              as String,
      foodType: null == foodType
          ? _value.foodType
          : foodType // ignore: cast_nullable_to_non_nullable
              as FoodTypeRemote,
      name: null == name
          ? _value.name
          : name // ignore: cast_nullable_to_non_nullable
              as String,
      barcode: null == barcode
          ? _value.barcode
          : barcode // ignore: cast_nullable_to_non_nullable
              as String,
      brandName: null == brandName
          ? _value.brandName
          : brandName // ignore: cast_nullable_to_non_nullable
              as String,
      description: null == description
          ? _value.description
          : description // ignore: cast_nullable_to_non_nullable
              as String,
      nationality: null == nationality
          ? _value.nationality
          : nationality // ignore: cast_nullable_to_non_nullable
              as String,
      image: null == image
          ? _value.image
          : image // ignore: cast_nullable_to_non_nullable
              as String,
      servingAmount: null == servingAmount
          ? _value._servingAmount
          : servingAmount // ignore: cast_nullable_to_non_nullable
              as List<double>,
      unit: null == unit
          ? _value._unit
          : unit // ignore: cast_nullable_to_non_nullable
              as List<String>,
      ingredients: null == ingredients
          ? _value._ingredients
          : ingredients // ignore: cast_nullable_to_non_nullable
              as List<String>,
      servingIngredientsCount: null == servingIngredientsCount
          ? _value._servingIngredientsCount
          : servingIngredientsCount // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      recipe: null == recipe
          ? _value.recipe
          : recipe // ignore: cast_nullable_to_non_nullable
              as String,
      units: null == units
          ? _value._units
          : units // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      calorie: null == calorie
          ? _value._calorie
          : calorie // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      protein: null == protein
          ? _value._protein
          : protein // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      carb: null == carb
          ? _value._carb
          : carb // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      fat: null == fat
          ? _value._fat
          : fat // ignore: cast_nullable_to_non_nullable
              as List<List<String>>,
      diets: null == diets
          ? _value._diets
          : diets // ignore: cast_nullable_to_non_nullable
              as List<String>,
      allergies: null == allergies
          ? _value._allergies
          : allergies // ignore: cast_nullable_to_non_nullable
              as List<String>,
      count: null == count
          ? _value.count
          : count // ignore: cast_nullable_to_non_nullable
              as double,
      prepTime: null == prepTime
          ? _value.prepTime
          : prepTime // ignore: cast_nullable_to_non_nullable
              as String,
      createdFromFatSecretRecipes: null == createdFromFatSecretRecipes
          ? _value.createdFromFatSecretRecipes
          : createdFromFatSecretRecipes // ignore: cast_nullable_to_non_nullable
              as int,
    ));
  }
}

/// @nodoc

class _$GenericFoodRemoteImpl implements _GenericFoodRemote {
  _$GenericFoodRemoteImpl(
      {this.id = '',
      this.foodType = FoodTypeRemote.meal,
      this.name = '',
      this.barcode = '',
      this.brandName = '',
      this.description = '',
      this.nationality = '',
      this.image = '',
      final List<double> servingAmount = const [],
      final List<String> unit = const [],
      final List<String> ingredients = const [],
      final List<List<String>> servingIngredientsCount = const [],
      this.recipe = '',
      final List<List<String>> units = const [],
      final List<List<String>> calorie = const [],
      final List<List<String>> protein = const [],
      final List<List<String>> carb = const [],
      final List<List<String>> fat = const [],
      final List<String> diets = const [],
      final List<String> allergies = const [],
      this.count = 0.0,
      this.prepTime = '',
      this.createdFromFatSecretRecipes = 1})
      : _servingAmount = servingAmount,
        _unit = unit,
        _ingredients = ingredients,
        _servingIngredientsCount = servingIngredientsCount,
        _units = units,
        _calorie = calorie,
        _protein = protein,
        _carb = carb,
        _fat = fat,
        _diets = diets,
        _allergies = allergies;

  @override
  @JsonKey()
  final String id;
  @override
  @JsonKey()
  final FoodTypeRemote foodType;
  @override
  @JsonKey()
  final String name;
  @override
  @JsonKey()
  final String barcode;
  @override
  @JsonKey()
  final String brandName;
  @override
  @JsonKey()
  final String description;
  @override
  @JsonKey()
  final String nationality;
  @override
  @JsonKey()
  final String image;
  final List<double> _servingAmount;
  @override
  @JsonKey()
  List<double> get servingAmount {
    if (_servingAmount is EqualUnmodifiableListView) return _servingAmount;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_servingAmount);
  }

  final List<String> _unit;
  @override
  @JsonKey()
  List<String> get unit {
    if (_unit is EqualUnmodifiableListView) return _unit;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_unit);
  }

  final List<String> _ingredients;
  @override
  @JsonKey()
  List<String> get ingredients {
    if (_ingredients is EqualUnmodifiableListView) return _ingredients;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_ingredients);
  }

  final List<List<String>> _servingIngredientsCount;
  @override
  @JsonKey()
  List<List<String>> get servingIngredientsCount {
    if (_servingIngredientsCount is EqualUnmodifiableListView)
      return _servingIngredientsCount;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_servingIngredientsCount);
  }

  @override
  @JsonKey()
  final String recipe;
  final List<List<String>> _units;
  @override
  @JsonKey()
  List<List<String>> get units {
    if (_units is EqualUnmodifiableListView) return _units;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_units);
  }

  final List<List<String>> _calorie;
  @override
  @JsonKey()
  List<List<String>> get calorie {
    if (_calorie is EqualUnmodifiableListView) return _calorie;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_calorie);
  }

  final List<List<String>> _protein;
  @override
  @JsonKey()
  List<List<String>> get protein {
    if (_protein is EqualUnmodifiableListView) return _protein;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_protein);
  }

  final List<List<String>> _carb;
  @override
  @JsonKey()
  List<List<String>> get carb {
    if (_carb is EqualUnmodifiableListView) return _carb;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_carb);
  }

  final List<List<String>> _fat;
  @override
  @JsonKey()
  List<List<String>> get fat {
    if (_fat is EqualUnmodifiableListView) return _fat;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_fat);
  }

  final List<String> _diets;
  @override
  @JsonKey()
  List<String> get diets {
    if (_diets is EqualUnmodifiableListView) return _diets;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_diets);
  }

  final List<String> _allergies;
  @override
  @JsonKey()
  List<String> get allergies {
    if (_allergies is EqualUnmodifiableListView) return _allergies;
    // ignore: implicit_dynamic_type
    return EqualUnmodifiableListView(_allergies);
  }

  @override
  @JsonKey()
  final double count;
  @override
  @JsonKey()
  final String prepTime;
  @override
  @JsonKey()
  final int createdFromFatSecretRecipes;

  @override
  String toString() {
    return 'GenericFoodRemote(id: $id, foodType: $foodType, name: $name, barcode: $barcode, brandName: $brandName, description: $description, nationality: $nationality, image: $image, servingAmount: $servingAmount, unit: $unit, ingredients: $ingredients, servingIngredientsCount: $servingIngredientsCount, recipe: $recipe, units: $units, calorie: $calorie, protein: $protein, carb: $carb, fat: $fat, diets: $diets, allergies: $allergies, count: $count, prepTime: $prepTime, createdFromFatSecretRecipes: $createdFromFatSecretRecipes)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$GenericFoodRemoteImpl &&
            (identical(other.id, id) || other.id == id) &&
            (identical(other.foodType, foodType) ||
                other.foodType == foodType) &&
            (identical(other.name, name) || other.name == name) &&
            (identical(other.barcode, barcode) || other.barcode == barcode) &&
            (identical(other.brandName, brandName) ||
                other.brandName == brandName) &&
            (identical(other.description, description) ||
                other.description == description) &&
            (identical(other.nationality, nationality) ||
                other.nationality == nationality) &&
            (identical(other.image, image) || other.image == image) &&
            const DeepCollectionEquality()
                .equals(other._servingAmount, _servingAmount) &&
            const DeepCollectionEquality().equals(other._unit, _unit) &&
            const DeepCollectionEquality()
                .equals(other._ingredients, _ingredients) &&
            const DeepCollectionEquality().equals(
                other._servingIngredientsCount, _servingIngredientsCount) &&
            (identical(other.recipe, recipe) || other.recipe == recipe) &&
            const DeepCollectionEquality().equals(other._units, _units) &&
            const DeepCollectionEquality().equals(other._calorie, _calorie) &&
            const DeepCollectionEquality().equals(other._protein, _protein) &&
            const DeepCollectionEquality().equals(other._carb, _carb) &&
            const DeepCollectionEquality().equals(other._fat, _fat) &&
            const DeepCollectionEquality().equals(other._diets, _diets) &&
            const DeepCollectionEquality()
                .equals(other._allergies, _allergies) &&
            (identical(other.count, count) || other.count == count) &&
            (identical(other.prepTime, prepTime) ||
                other.prepTime == prepTime) &&
            (identical(other.createdFromFatSecretRecipes,
                    createdFromFatSecretRecipes) ||
                other.createdFromFatSecretRecipes ==
                    createdFromFatSecretRecipes));
  }

  @override
  int get hashCode => Object.hashAll([
        runtimeType,
        id,
        foodType,
        name,
        barcode,
        brandName,
        description,
        nationality,
        image,
        const DeepCollectionEquality().hash(_servingAmount),
        const DeepCollectionEquality().hash(_unit),
        const DeepCollectionEquality().hash(_ingredients),
        const DeepCollectionEquality().hash(_servingIngredientsCount),
        recipe,
        const DeepCollectionEquality().hash(_units),
        const DeepCollectionEquality().hash(_calorie),
        const DeepCollectionEquality().hash(_protein),
        const DeepCollectionEquality().hash(_carb),
        const DeepCollectionEquality().hash(_fat),
        const DeepCollectionEquality().hash(_diets),
        const DeepCollectionEquality().hash(_allergies),
        count,
        prepTime,
        createdFromFatSecretRecipes
      ]);

  @JsonKey(ignore: true)
  @override
  @pragma('vm:prefer-inline')
  _$$GenericFoodRemoteImplCopyWith<_$GenericFoodRemoteImpl> get copyWith =>
      __$$GenericFoodRemoteImplCopyWithImpl<_$GenericFoodRemoteImpl>(
          this, _$identity);
}

abstract class _GenericFoodRemote implements GenericFoodRemote {
  factory _GenericFoodRemote(
      {final String id,
      final FoodTypeRemote foodType,
      final String name,
      final String barcode,
      final String brandName,
      final String description,
      final String nationality,
      final String image,
      final List<double> servingAmount,
      final List<String> unit,
      final List<String> ingredients,
      final List<List<String>> servingIngredientsCount,
      final String recipe,
      final List<List<String>> units,
      final List<List<String>> calorie,
      final List<List<String>> protein,
      final List<List<String>> carb,
      final List<List<String>> fat,
      final List<String> diets,
      final List<String> allergies,
      final double count,
      final String prepTime,
      final int createdFromFatSecretRecipes}) = _$GenericFoodRemoteImpl;

  @override
  String get id;
  @override
  FoodTypeRemote get foodType;
  @override
  String get name;
  @override
  String get barcode;
  @override
  String get brandName;
  @override
  String get description;
  @override
  String get nationality;
  @override
  String get image;
  @override
  List<double> get servingAmount;
  @override
  List<String> get unit;
  @override
  List<String> get ingredients;
  @override
  List<List<String>> get servingIngredientsCount;
  @override
  String get recipe;
  @override
  List<List<String>> get units;
  @override
  List<List<String>> get calorie;
  @override
  List<List<String>> get protein;
  @override
  List<List<String>> get carb;
  @override
  List<List<String>> get fat;
  @override
  List<String> get diets;
  @override
  List<String> get allergies;
  @override
  double get count;
  @override
  String get prepTime;
  @override
  int get createdFromFatSecretRecipes;
  @override
  @JsonKey(ignore: true)
  _$$GenericFoodRemoteImplCopyWith<_$GenericFoodRemoteImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
